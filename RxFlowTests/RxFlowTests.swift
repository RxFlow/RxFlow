...
class RxFlowTests: XCTestCase {
    ...
    func testPost() {
        ...
        RxFlow().target(postURL).post(data, parser: SwiftyJSONParser).subscribeNext {
            json, _ in result = json
            expectation.fulfill()
        }.addDisposableTo(disposeBag)
    ...
    func testPut() {
        ...
        RxFlow().target(putURL).put(data, parser: SwiftyJSONParser).subscribeNext {
            json, _ in result = json
            expectation.fulfill()
        }.addDisposableTo(disposeBag)
    ...
    func testDelete() {
        ...
        RxFlow().target(deleteURL).delete().subscribeNext {
            response, _ in result = response
            expectation.fulfill()
        }.addDisposableTo(disposeBag)
    ...
    func testHead() {
        ...
        RxFlow().target(getURL).head().subscribeNext {
            headers in expectation.fulfill()
        }.addDisposableTo(disposeBag)
    ...
    func testOptions() {
        ...
        RxFlow().target(getURL).options().subscribeNext {
    ...
    func testPatch() {
        ...
        RxFlow().target(patchURL).patch(data, parser: SwiftyJSONParser).subscribeNext {
            json, _ in result = json
            expectation.fulfill()
        }.addDisposableTo(disposeBag)
    ...
    func testSingleQueryParameter() {
        ...
        RxFlow().target(getURL).parameter("name", value: "value").get().subscribeNext {
            json, _ in result = json
            expectation.fulfill()
        }.addDisposableTo(disposeBag)
    ...
    func testMultipleQueryParamaters() {
        ...
        RxFlow().target(getURL).parameters(parameters).get().subscribeNext {
            json, _ in result = json
            expectation.fulfill()
        }.addDisposableTo(disposeBag)
    ...
    func testSingleHeader() {
        ...
        RxFlow().target(getURL).header("Api-Key", value: "12345").get().subscribeNext {
            json, _ in result = json
            expectation.fulfill()
        }.addDisposableTo(disposeBag)
    ...
    func testMultipleHeaders() {
        ...
        RxFlow().target(getURL).headers(headers).get().subscribeNext {
            json, _ in result = json
            expectation.fulfill()
        }.addDisposableTo(disposeBag)
    ...
    func testCommunicationError() {
        ...
        RxFlow().target("invalid_address").get().subscribeError {
            error in do {
                throw error
            } catch FlowError.CommunicationError(_) {
                expectation.fulfill()
            } catch {
            }
        }.addDisposableTo(disposeBag)
    ...
    func testUnsupportedStatusCode() {
        ...
        let expectation = expectationWithDescription("Status code 400 should result in UnsupportedStatusCode")

        RxFlow().target("https://httpbin.org/status/400").get().subscribeError {
            error in do {
                throw error
            } catch FlowError.UnsupportedStatusCode(_) {
                expectation.fulfill()
            } catch {
            }
        }.addDisposableTo(disposeBag)
    ...
    func testParsingIsNotPerformedOnMainThread() {
        ...
        let expectation = expectationWithDescription("Parsing should not be performed on main thread")
        var isMain = true

        RxFlow().target(getURL).get {
            (data) -> Void in isMain = NSThread.isMainThread()
        }.observeOn(MainScheduler.instance).subscribeNext {
            _, _ in expectation.fulfill()
        }.addDisposableTo(disposeBag)
    ...
    func testCallbackIsDoneOnMainThread() {
    ...
    func testParseError() {
        ...
        let expectation = expectationWithDescription("Error should have been thrown while parsing")

        RxFlow().target(getURL).get {
            _ in throw FlowError.ParseError(nil)
        }.subscribeError {
            error in do {
                throw error
            } catch FlowError.ParseError(_) {
                expectation.fulfill()
            } catch {
            }
        }.addDisposableTo(disposeBag)
    ...
    func testRetry() {
        ...
        RxFlow().target("https://httpbin.org/status/400", retries: retries).get().subscribeError {
